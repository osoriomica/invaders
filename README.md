...
| Shooter Options              | Click on selected shooter                                 | On clicking each shooter on the modal, the game controls are update with selected shooter                                                                                                                                                         | Pass         |
| Invaders: Start/Pause/Resume | Click button                                              | If modal is closed with esc, the button reads Start. On click it starts the game interval and turns to Pause. On click, it pauses the game and prevents shooting event and turns to Resume. On click it resumes the game and turns back to Pause. | Pass         |
...
| Move invaders                | On game play                                              | On start, the invaders start moving left to right and downwards.                                                                                                                                                                                  | Pass         |
...
| Remove Invaders              | Shoot the invaders on game play                           | When hit by the flash, the invader is removed from the grid and the score is increased by one.                                                                                                                                                    | Pass         |
| padScore                     | On game play                                              | On shooting an invader, the score’s format is displayed in 3 digits                                                                                                                                                                               | Pass         |
| Game Over                    | On game play                                              | If no more invaders left, the game is won. 

If the invaders touch the shooter or reach the bottom row, the game is lost. A message is displayed to let the user know the result of the game.                                                     | Pass         |
| Footer:                      |                                                           |                                                                                                                                                                                                                                                   |              |
